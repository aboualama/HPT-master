              ...
              <div>
                  ...
                  @foreach ( config('translatable.locales') as $lang)
                    ...
                    <div class="{{ $loop->last ? 'col-md-12' : 'col-md-6'}}">
                      ...
                      <input type="text" class="form-control" name="{{$lang}}[title]"
                             value="{{$record->translate($lang)->title}}" required>
                ...
                <div class="form-group row">
                  @foreach ( config('translatable.locales') as $lang)
                    <div class="{{ $loop->last ? 'col-md-12' : 'col-md-6'}}">
                      <label>{{$lang}}</label>
                      <input type="text" class="form-control" name="{{$lang}}[entro]"
                             value="{{$record->translate($lang)->entro}}" required>
                ...
                <div id="sec" class="secrow">
                  ...
                  <h3>Perception</h3>
                  @php($m = is_array(json_decode($record->msg)))
                  @php($c = $m ? count(json_decode($record->msg)):0)
                  @for ($i = 0 ; $i < $m ; $i++)
                    <hr>
                    <div class="form-group row">
                      <div class="col-md-12">
                        ...
                        <input type="text" class="form-control" name="sec[]"
                               value="{{ json_decode($record->msg , true)[$i]['sec'] }}" required>
                      ...
                      @foreach ( config('translatable.locales') as $lang)
                        <div class="{{ $loop->last ? 'col-md-12' : 'col-md-6'}}">
                          <label>{{$lang}}</label>
                          <input type="text" class="form-control" name="{{$lang}}[msg][]"
                                 value="{{json_decode($record->translate($lang)->msg , true)[$i]['msg']}}" required>
                          <small id="{{$lang}}_msg_{{$i}}_error"
                                 class="form-text text-danger center small_error"> </small>
                        </div>
                      @endforeach
                    </div>
                  @endfor
